document.addEventListener('DOMContentLoaded', () => {
  ...
  // ---- Tesoura a "cortar" as linhas de "Como funciona" (presa ao scroll) ----
  ...
  if (cutEl) {
    ...
    mmCut.add('(min-width: 961px)', () => {
      ...
      const update = (p) => {
        ...
        const openness = (1 + Math.cos(p * Math.PI * 2 * 5)) / 2; // 1 = aberta, 0 = fechada
        const k = 1 - openness;
      ...
      // A secção "Como funciona" fica FIXA no centro do ecrã enquanto a
      // tesoura corta as linhas; só quando ela chega ao fim é que o scroll
      // volta a levar a página. O proxy dá o progresso suavizado (scrub).
      const proxy = { p: 0 };
      gsap.to(proxy, {
        p: 1,
        ease: 'none',
  ...
  document.querySelectorAll('.reveal-up').forEach(el => {
    gsap.fromTo(el,
      { opacity: 0, y: 56 },
      {
        ...
        scrollTrigger: { trigger: el, start: 'top 96%', end: el.closest('.processo') ? 'top 80%' : 'top 55%', scrub: 0.4 }
      }
    );
  });
